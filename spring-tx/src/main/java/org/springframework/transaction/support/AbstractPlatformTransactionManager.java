...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// <1> 如果没有 @Transactional 注解对应的元信息，则创建一个默认的 TransactionDefinition 对象
		TransactionDefinition def = (definition != null ? definition : TransactionDefinition.withDefaults());

		// <2> 先从当前事务管理器中获取 DataSource 对象，然后尝试以它作为一个 Key 从一个 ThreadLocal 的 Map 中获取对应的 ConnectionHolder 连接对象
	...
	private TransactionStatus handleExistingTransaction(
		...
		// <4> 否则，如果是 **NESTED** 事务传播级别（执行一个嵌套事务），还是使用当前线程的事务，不过设置了保存点，
		// 相当于一个嵌套事务，在 Mysql 中是采用 SAVEPOINT 来实现的
